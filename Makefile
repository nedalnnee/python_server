...
# already have the $(CONFIGURATION) files.
# The "or true" tells 'make' not to complain about shutting
# down with Ctrl-C
# 
run:	pageserver/pageserver.py
	python3 pageserver/pageserver.py 

# When run as "make start" and "make stop", all arguments
# come from the configuration file. The 'start.sh' and
# 'stop.sh' scripts can also be run directly, and then
# start.sh can take other command line arguments.
#
